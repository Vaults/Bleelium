...
import {rewriteAttributes, isEqual} from '/server/imports/util.js';
    ...
    f: function (args) {
        ...
        var getSpaceList = function(){
            ...
            for (var item in args.data.contextResponses) {
                var space = args.data.contextResponses[item].contextElement;
                if (!tempObj[space.attributes.lotId]) {
                    tempObj[space.attributes.lotId] = [];
                }
                tempObj[space.attributes.lotId].push(space);
            }
            return tempObj;
        }

        var update = getSpaceList();
