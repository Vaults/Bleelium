    ...
    HTTP.call('GET', 'http://feeds.livep2000.nl/?r=22&d=1,2,3', function (error, response) {
        if (error) {
            ...
            xml2js.parseString(response.content, function (err, result) {
                ...
                HTTP.call('GET', 'http://131.155.70.152:1026/v1/contextEntityTypes/P2000?orderBy=!publish_date&limit=1000', function (error, response) {
                    if (error) {
                        console.log(error);
                    } else {
                        response = rewriteAttributes(response);
                        for (item in response.data.contextResponses) {
                            if(collectionWrapper['P2000'].findOne(response.data.contextResponses[item].contextElement)){

                                   // !collectionWrapper['P2000'].findOne(response.data.contextResponses[item].contextElement)){
                            }else {
                                console.log(response.data.contextResponses[item].contextElement);
